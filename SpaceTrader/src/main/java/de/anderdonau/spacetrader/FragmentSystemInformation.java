...
public class FragmentSystemInformation extends MyFragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		textView.setText(main.GetSystemName(CURSYSTEM.nameIndex));
		...
		textView = (TextView) rootView.findViewById(R.id.strSysInfoSpecies);
		textView.setText(gameState.getAlienSpeciesName(CURSYSTEM.dominantSpecies));
		textView = (TextView) rootView.findViewById(R.id.strSysInfoGuild);
		textView.setText(gameState.getGuildName(CURSYSTEM.controllingGuild));
		textView = (TextView) rootView.findViewById(R.id.strSysInfoConflict);
		textView.setText(CURSYSTEM.conflictLevel + "/10");
		textView = (TextView) rootView.findViewById(R.id.strSysInfoDeepIntel);
		textView.setText(gameState.systemIntelSummary(CURSYSTEM));
	...
	void showNewspaperPopup() {
		...
		if (NewsPaperNames[CURSYSTEM.politics][WarpSystem % GameState.MAXMASTHEADS].startsWith("*")) {
			masthead = String.format("The %s %s", main.GetSystemName(CURSYSTEM.nameIndex),
			...
			"+")) {
			masthead = String.format("%s %s", main.GetSystemName(CURSYSTEM.nameIndex),
				NewsPaperNames[CURSYSTEM.politics][WarpSystem % GameState.MAXMASTHEADS].substring(2));
		} else {
		...
		// Space Trader 2 system texture: use the expanded local data to make newspapers
		// feel like they know something about the route, guilds, and current pressure.
		if (CURSYSTEM != null) {
			news += "\nPort Authority Advisory: " + gameState.systemTravelAdvisory(CURSYSTEM);
			news += "\nDockside Rumor: " + gameState.systemRumorLine(CURSYSTEM);
		}
		...
		if (gameState.PoliceRecordScore <= GameState.VILLAINSCORE) {
			...
			switch (j) {
				case 0:
					news +=
						"\nPolice Warning: " + gameState.NameCommander + " Will Dock At " + main.GetSystemName(CURSYSTEM.nameIndex) + "!";
					break;
				case 1:
					news +=
						"\nNotorious Criminal " + gameState.NameCommander + " Sighted In " + main.GetSystemName(CURSYSTEM.nameIndex) + "!";
					break;
				case 2:
		...
		for (i = 0; i < GameState.MAXSOLARSYSTEM; i++) {
			if (i != COMMANDER.curSystem &&
				...
				if (gameState.SolarSystem[i].special == GameState.MOONFORSALE) {
					news += "\nSeller in " + main.GetSystemName(i) + " System has Utopian Moon available.";
				}
				if (gameState.SolarSystem[i].special == GameState.BUYTRIBBLE) {
					news +=
						"\nCollector in " + main.GetSystemName(i) + " System seeks to purchase Tribbles.";
				}
				...
				if (gameState.GetRandom(
					...
					news += " in the " + main.GetSystemName(i) + " System.";
					realNews = true;
				}
			}
		}
		...
		switch (CURSYSTEM.special) {
			case GameState.GETREACTOR:
				if (gameState.Ship.FilledCargoBays() > gameState.Ship.TotalCargoBays() - 15) {
					...
					popup = new Popup(main, "Wild Won't Stay Aboard", String.format(
						...
						main.GetSystemName(CURSYSTEM.nameIndex)), "", "Goodbye Wild", "Leave Reactor",
						new Popup.buttonCallback() {
							@Override
							public void execute(Popup popup, View view) {
			...
			case GameState.TRANSPORTWILD:
				...
				if (!gameState.Ship.HasWeapon(GameState.BEAMLASERWEAPON, false)) {
					popup = new Popup(main, "Wild Won't Stay Aboard", String.format(
						...
						main.GetSystemName(CURSYSTEM.nameIndex)), "", "OK", main.cbShowNextPopup);
					main.addPopup(popup);
					return;
				}
			...
			case GameState.INSTALLLIGHTNINGSHIELD:
				FirstEmptySlot = gameState.GetFirstEmptySlot(
					gameState.EffectiveShieldSlots(), gameState.Ship.shield);
				if (FirstEmptySlot < 0) {
			...
			case GameState.GETSPECIALLASER:
				FirstEmptySlot = gameState.GetFirstEmptySlot(
					gameState.EffectiveWeaponSlots(), gameState.Ship.weapon);
				if (FirstEmptySlot < 0) {
					popup = new Popup(main, "Not Enough Slots",
						"You have already filled all of your available slots for this type of item.", "", "OK",
						main.cbShowNextPopup);
